...
class CooperativeCoevolutionaryAlgorithm():
    ...
    def evaluateTeam(self, team):
        ...
        #TODO Make it so critics can be used here directly with the help of multithreading
        # May have to modify critics or use a special pickle library (dill) to make this work
    ...
    def overwriteEvalWithCritic(self, eval_infos):
        ...
        for eval_info in eval_infos:
            ...
            for idx, t in enumerate(jt):
                t = np.array(t)
                val, vec = self.critic.evaluate(t, idx)
                agent_rewards.append(val)
                agent_rewards_vec.append(vec)
            eval_info.rewards = RewardInfo(
                agent_rewards = [r for r in agent_rewards],
                agent_rewards_vec = [r for r in agent_rewards_vec],
                G_vec = eval_info.rewards.G_vec,
                G = eval_info.rewards.G
            )
